...
页面壳层统一采用 `de-employee-page` / `de-employee-shell` / KPI 卡片与品牌色（靛紫 `#4f46e5`）交互；中英文界面使用同一套词条。组织、工作区、模型和用户创建的资产名称保持原样，不进行机器翻译。视觉验收见 [`docs/视觉设计规范.md`](./docs/视觉设计规范.md)；架构与模块说明见 [`docs/`](./docs/)。
