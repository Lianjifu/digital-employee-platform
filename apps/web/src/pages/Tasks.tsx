...
const COLUMNS: { key: TaskStatus; label: string; color: string }[] = [
  { key: 'in_progress', label: '进行中', color: 'border-[var(--color-primary)]' },
  { key: 'review', label: '待复核', color: 'border-amber-500' },
  { key: 'completed', label: '已完成', color: 'border-emerald-500' },
  { key: 'archived', label: '已归档', color: 'border-[var(--color-border)]' },
];
  ...
  return (
    <div className="grid h-full grid-cols-[1fr_360px] divide-x divide-[var(--color-border)]">
      ...
      <div className="flex h-full flex-col overflow-hidden">
        {/* 顶部 KPI */}
        <div className="grid grid-cols-4 gap-3 border-b border-[var(--color-border)] p-4">
          <Stat label="今日任务" value={tasks?.length ?? 0} sub="次/日" tone="primary" />
          <Stat label="平均执行" value="38" sub="min" tone="primary" />
        ...
        <div className="grid flex-1 grid-cols-4 gap-3 overflow-x-auto p-4">
          {grouped.map((col) => (
            <div key={col.key} className={cn('flex min-w-[260px] flex-col rounded-lg border-t-2 bg-[var(--color-surface-1)]', col.color)}>
              <div className="flex items-center justify-between border-b border-[var(--color-border)] px-3 py-2">
                <div className="flex items-center gap-1.5 text-sm font-medium">
                  {col.key === 'in_progress' && <Clock className="h-3.5 w-3.5 text-[var(--color-primary)]" />}
                  ...
                  {col.key === 'archived' && <RotateCcw className="h-3.5 w-3.5 text-[var(--color-text-muted)]" />}
                  {col.label}
                </div>
                <Badge tone={col.items.length > 0 ? 'primary' : 'neutral'}>{col.items.length}</Badge>
              </div>
              ...
              <div className="flex-1 space-y-2 overflow-y-auto p-2">
                ...
                {col.items.map((t) => (
                  <button
                    ...
                    className={cn(
                      'w-full rounded-md border border-[var(--color-border)] bg-[var(--color-surface-2)] p-3 text-left text-xs transition-all hover:border-[var(--color-primary)]',
                      activeId === t.id && 'border-[var(--color-primary)] ring-2 ring-[var(--color-primary)]/30',
                    )}
                  >
                    <div className="mb-1 flex items-center gap-1.5">
                      <Badge tone={PRIORITY_TONE[t.priority]}>{t.priority}</Badge>
                      <span className="font-mono text-[10px] text-[var(--color-text-muted)]">{t.code}</span>
                    </div>
                    ...
                    {t.slaRemainingMin !== undefined && (
                      <div className={cn('mt-1.5 flex items-center gap-1 text-[10px]', t.slaRemainingMin < 0 ? 'text-rose-500' : t.slaRemainingMin < 60 ? 'text-amber-500' : 'text-[var(--color-text-muted)]')}>
                    ...
                    <div className="mt-1.5 flex flex-wrap gap-1">
                      {t.tags.slice(0, 3).map((tag) => (
                        <span key={tag} className="rounded bg-[var(--color-surface-3)] px-1 py-0.5 text-[10px] text-[var(--color-text-muted)]">
                          #{tag}
                        </span>
                      ))}
                    </div>
        ...
        {active ? (
          <>
            <div className="border-b border-[var(--color-border)] p-4">
              <div className="mb-2 flex items-center gap-2">
                ...
                <span className="font-mono text-xs text-[var(--color-text-muted)]">{active.code}</span>
            ...
            <div className="flex-1 overflow-y-auto p-4">
              <div className="mb-4">
                <div className="mb-1 text-xs text-[var(--color-text-muted)]">步骤进度</div>
              ...
              <div className="mb-4">
                ...
                <div className="space-y-2 text-xs">
                  {[
                    ...
                    <div key={i} className="flex gap-2 border-l border-[var(--color-border)] pl-3">
                      <div className="text-base">{e.icon}</div>
                      <div className="flex-1">
                        <div className="font-mono text-[10px] text-[var(--color-text-muted)]">{e.t}</div>
                        <div>{e.text}</div>
                      </div>
                    </div>
                  ))}
              ...
              <div className="mb-4">
                <div className="mb-1 text-xs text-[var(--color-text-muted)]">标签</div>
                <div className="flex flex-wrap gap-1">
                  {active.tags.map((tag) => (
                    <span key={tag} className="rounded bg-[var(--color-surface-2)] px-1.5 py-0.5 text-[10px]">#{tag}</span>
                  ))}
                </div>
              </div>
            </div>
          ...
          <div className="grid flex-1 place-items-center text-center">
            <div>
              ...
              <div className="text-sm text-[var(--color-text-muted)]">选择左侧任务查看详情</div>
            </div>
          </div>
        )}
      </aside>
...
function Stat({ label, value, sub, tone, icon }: { label: string; value: any; sub?: string; tone: 'primary' | 'success' | 'warn'; icon?: React.ReactNode }) {
  const c = tone === 'success' ? 'text-emerald-500' : tone === 'warn' ? 'text-amber-500' : 'text-[var(--color-primary)]';
  return (
    <Card>
