        ...
        <div className="space-y-1">
          {(list ?? []).map((w) => (
            <div
              key={w.id}
              className={cn(
                'flex cursor-pointer items-center gap-2 rounded-md border border-transparent p-2 hover:border-[var(--color-border)] hover:bg-[var(--color-surface-2)]',
                w.id === active?.id && 'border-[var(--color-primary)] bg-[var(--color-primary)]/10',
              )}
            >
              <div className="grid h-8 w-8 place-items-center rounded-md bg-[var(--color-surface-3)]">
          ...
          <>
                ...
                <Card key={s.label}>
                  <CardBody className="flex items-center justify-between">
                    <div>
                      ...
                      <div className={cn('mt-0.5 text-xl font-semibold', s.tone === 'success' ? 'text-emerald-500' : 'text-[var(--color-primary)]')}>
            ...
            <Card className="mb-4">
              <CardHeader>
                ...
                <span className="text-xs text-[var(--color-text-muted)]">RBAC + 字段级</span>
              </CardHeader>
              <CardBody>
                ...
                <Card key={c.title}>
                  <CardBody className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <div className="mt-0.5 text-[11px] text-[var(--color-text-muted)]">{c.content}</div>
                    </div>
                  </CardBody>
                </Card>
              ))}
      ...
      <aside className="space-y-3 overflow-y-auto p-4">
        <Card>
          ...
          <CardBody className="space-y-2 text-xs">
            <div className="text-[var(--color-text-muted)]">支持多企业、多团队隔离 · 独立 Agent / 知识 / 工具</div>
        ...
        <Card>
          ...
          <CardBody className="space-y-1.5 text-xs">
            {['新增成员 张睿', '升级 Enterprise Plus', '完成等保 3 审计'].map((e, i) => (
              <div key={i} className="rounded bg-[var(--color-surface-2)] px-2 py-1.5">{e}</div>
            ))}
          </CardBody>
        </Card>
      </aside>
