...
func authorizeToolCall(reg []registeredTool, call toolCallRequest) (*registeredTool, *toolExecResult) {
	...
	if t.RequiresApproval {
		return t, &toolExecResult{
			Status: "denied", Error: "工具需要双重审批后才能执行：" + t.Name, Permission: "approval_required",
		}
	}
	return t, nil
}
...
func (s *Server) runSkillTool(ctx toolRunContext, t *registeredTool, call toolCallRequest, started time.Time) toolExecResult {
	...
	input := coalesce(str(call.Args["input"]), coalesce(str(call.Args["command"]), ctx.UserMessage))
	...
	for _, item := range s.Store.Skills {
		...
		if strings.EqualFold(name, t.Name) || slugToolName(name) == slugToolName(t.Name) {
			sk = item
			break
		}
	}
	...
	// Reuse policy + runtime path via internal call (same as HTTP executeSkill core).
	id := ctx.Viewer
	if id == nil {
