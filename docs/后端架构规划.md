...
> **契约真相源**：[`backend/api/routes.md`](../backend/api/routes.md) · [`backend/api/contract-gap.md`](../backend/api/contract-gap.md) ·（对照）`frontend/packages/api/src/mock.ts` · [ADR-013](./adr/ADR-013-agent-os-kernel.md) · [ADR-014 作用域三层](./adr/ADR-014-scope-layers.md) · [环境与数据模式](./环境与数据模式.md) · [近端实施方案](./实施方案-对齐生产写路径.md)  
