...
> **版本**：v1.3（2026-08-01）  
> **状态**：逻辑架构 + 工程契约基线（后端未在本仓交付）  
> **前提**：前端 L0（控制面 + Mock）已交付；契约真相源 `frontend/packages/api/src/mock.ts`  
> **配套产品文档**：[架构文档](./数字员工平台-架构文档.md) · [功能模块](./数字员工平台-功能模块文档.md) · [技术规格](./数字员工平台-技术规格说明.md)
...
**Callers** = 控制台 · OpenAPI · 企微/邮件入站 · Webhook · SDK（外部调用方，≠ `cap-channel`）。
...
┌─────────────────────────────────────────────────────────────────────────┐
│  Callers                                                                │
│  控制台 · OpenAPI · 企微/邮件入站 · Webhook · SDK                         │
└─────────────────────────────────┬───────────────────────────────────────┘
                                  │
                                  ▼
┌─ ① api · 入口 ──────────────────────────────────────────────────────────┐
...
| 阶段 | 进程包 | 内含边界 |
|---|---|---|
| A | de-gateway + de-core | platform · policy · audit |
| B | de-control | collab · employee |
| C | capability-lite + AI | model · knowledge · agent-runtime · rag |
| D | 按隔离拆分 | workflow · skill · skill-runtime · memory · channel |
| E | de-ops；audit 查询扩缩 | 运营与合规导出 |
...
| 约定 | 说明 |
|---|---|
| 形态 | `backend/go/` + `backend/python/` + 共享 `api/proto/` |
...
原则：下游只引用**已发布**资产；数字员工是唯一岗位主体；流程须「发布技能」后进技能中心再装配；治理横切不生产能力。
...
一次「人对在岗数字员工发消息」时，产品模块如何协作（不含配置发布）：
...
|---|---|---|---|---|---|
| M01 | 运营总览 | Go | KPI/SLA/待办/成本/活动 | `/api/home/*` · operations | P1 |
...
| M05 | 数字员工 | Go+Py | 岗位、装配、评测、上岗 | `/api/digital-employees/*` | **P0** |
...
| **B 编排** | 数字员工 + 任务 + 双签 + 访问控制/零信任 | 可上岗、可处置任务 |
...
1. 冻结本规划为 ADR-013（服务边界 + 分层命名 + 契约基线）。  
2. 按 §9 建 `api/proto`：优先 `platform` · `policy` · `audit` · `collab` · `employee`。  
3. 阶段 A：`de-core` + gateway；错误码与角色强制按 §11 / §12.3 落地。  
4. 并行 `de-agent-runtime` / `de-rag` 健康检查 stub（Connect 互通）。  
5. 前端 `VITE_USE_MOCK=false` 联调阶段 A 路径。
...
> 原则：对外 REST 路径对齐 Mock；对内 Connect-RPC 包名 `de.<domain>.v1`。gateway 做路径 → 服务路由。
...
| 能力 | 对外 | 对内 |
|---|---|---|
| 专家协作发消息 | `POST /api/copilot/sessions/{id}/messages:stream`（SSE） | Connect bidi/server-stream → `de.runtime.v1.AgentRuntime/Run` |
| 网关 | L01 保持 SSE 扇出；不缓冲全量再转发 | runtime 事件映射为 §4.3 的 `delta/tool/done/error` |
...
| `E_DIGITAL_EMPLOYEE_*` | 数字员工配置/上岗 | 400/403/404 |
...
适用：数字员工上岗/受控配置变更、模型路由发布、知识/技能/渠道生产发布、`release-approvals` 流程。
...
| 项 | 约定 |
|---|---|
| 定位 | Mock 中技术运行/评测遗留面；**产品主路径为 `/api/digital-employees*`** |
| 后端 | 阶段 B 不新建独立 `de-agents`；评测/live calls 能力并入 `de-employee` + `de-agent-runtime` |
| 兼容 | 若需过渡，gateway 将 `/api/agents` **只读代理**到 employee/runtime，并标记 `Deprecation` 头 |
| 禁止 | 新功能不得只挂在 `/api/agents` |
...
| 主题 | 参见 |
|---|---|
| 多区域 cn-east/south、RTO/RPO | [架构文档](./数字员工平台-架构文档.md) §5 |
| 等保 3 + ISO、ADR 锁定 | [技术规格](./数字员工平台-技术规格说明.md) §13、架构文档 §7 |
...
|---|---|---|
| 逻辑架构 | **充分** | §1–§7 |
| 工程契约基线 | **已具备** | §9–§12（映射 · 所有权 · 错误/状态机 · 事件/安全/角色/晋升） |
| 运维数值与多活细规 | **仍弱（P2）** | 见下表；以技术规格/架构文档为准 |
| Mock 逐接口字段级对齐 | **进行中** | 前缀级已映射；字段以 proto 生成后契约测试锁定 |
