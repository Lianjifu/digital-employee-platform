...
> **版本**：v1.6（2026-08-17）  
> **状态**：逻辑架构 + 工程契约基线；**本仓已交付粗粒度后端联调**（内存态 + Python stubs）  
> **前提**：前端 L0 已交付；默认真实网关联调；Mock 为可选对照面  
> **契约真相源**：[`backend/api/routes.md`](../backend/api/routes.md) · [`backend/api/contract-gap.md`](../backend/api/contract-gap.md) ·（对照）`frontend/packages/api/src/mock.ts`  
...
|---|---|---|
| 逻辑架构 | **充分** | §1–§7 |
| 工程契约基线 | **已具备** | §9–§12；本仓粗粒度服务可联调 |
| 运维数值与多活细规 | **仍弱（P2）** | 见下表；以技术规格/架构文档为准 |
| 接口字段级对齐 | **进行中** | 见 `contract-gap.md`；字段以 proto 生成后契约测试锁定 |
