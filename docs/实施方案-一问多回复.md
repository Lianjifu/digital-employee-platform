...
| G4 | 可配置 `replyMode` | **平台默认 `segmented`**；员工/请求可显式改 `single` |
...
type ReplyMode =
  | 'single'      // 显式单条（合规/长报告）
  | 'segmented'   // 平台默认：终稿按规则拆成 2–N 条气泡
  | 'stepwise';   // Harness 阶段各发一条（计划/步骤/结论）
```

| 模式 | 触发点 | 典型员工 |
|------|--------|----------|
| `single` | 无拆段 | 需单泡输出的合规场景（显式配置） |
| `segmented` | `streamHarnessAnswer` 前拆 `[]string` | **全部数字员工默认** |
| `stepwise` | plan / multi-agent / 技能多步 | 文档整理、复杂任务 |

**解析顺序**：`POST body.replyMode` → `DE_COPILOT_REPLY_MODE` → `employee.runtime.replyMode` → 默认 **`segmented`**。
...
所有数字员工默认 `employee.runtime.replyMode = "segmented"`（`store/reply_mode.go` + 启动时 `EnsureEmployeesReplyModeDefaults`）；`de-office` 办公助手同样遵循该默认。
