...
export default function Channels() {
  ...
  return (
    <div className="channels-page h-full min-w-0 overflow-y-auto bg-[var(--bg-elevated)] p-3 md:p-4 lg:p-5">
      <div className="space-y-3">
        <section className="de-employee-shell overflow-hidden rounded-xl bg-[var(--surface-1)]">
          <div className="flex items-start justify-between gap-4 px-4 py-3.5 md:px-5">
            ...
            <Badge tone="warn" className="shrink-0">Mock 控制面演示 · 服务端负责真实凭据与授权</Badge>
          </div>
        ...
        <section className="grid grid-cols-2 gap-3 lg:grid-cols-3" aria-label="渠道摘要">
          <KpiCard label="活跃部署" value={overview.data?.activeDeployments ?? 0} sub="个" icon={Cloud} tone="brand" size="comfortable" />
          <KpiCard label="已发布策略" value={overview.data?.publishedPolicies ?? 0} sub="个" icon={Route} tone="brand" size="comfortable" />
          <KpiCard label="死信待处置" value={overview.data?.deadLetters ?? 0} sub="条" icon={AlertTriangle} tone={(overview.data?.deadLetters ?? 0) > 0 ? 'warn' : 'success'} size="comfortable" />
        </section>

        <section className="de-employee-shell overflow-hidden rounded-xl bg-[var(--surface-1)]">
          <div className="p-3 md:p-4">
            {loading ? (
              <div className="py-12 text-center text-sm text-[var(--text-muted)]">正在读取渠道控制面状态…</div>
            ) : tab === 'deployments' ? (
              <Deployments items={deployments.data ?? []} canWrite={canWrite} refs={policyRefs} onNew={() => setNewOpen(true)} onVerify={(id) => verify.mutate({ id }, { onSuccess: () => toast.success('渠道连通性验证通过'), onError: notifyError })} onDelete={setDeleteDeployment} />
            ) : tab === 'routing' ? (
              <Routing policies={policies.data ?? []} canWrite={canWrite} onOpen={setSelectedPolicy} />
            ) : tab === 'templates' ? (
              <Templates />
            ) : tab === 'health' ? (
              <Health overview={overview.data} />
            ) : tab === 'failures' ? (
              <Failures items={failures.data ?? []} />
            ) : (
...
function Deployments({ items, canWrite, refs, onNew, onVerify, onDelete }: { items: ChannelDeployment[]; canWrite: boolean; refs: Set<string>; onNew: () => void; onVerify: (id: string) => void; onDelete: (d: ChannelDeployment) => void }) {
  return (
    <div>
      ...
      <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
        {items.map((d) => {
          ...
          return (
            <article key={d.id} className="rounded-lg border border-[var(--border)] p-3">
              ...
              <p className="mt-3 font-mono text-[11px] text-[var(--text-muted)]">{d.credentialMasked} · {d.lastVerifiedAt ? '已验证' : '待验证'}</p>
              <div className="mt-3 flex gap-2">
                <Button size="sm" variant="secondary" disabled={!canWrite} onClick={() => onVerify(d.id)}><ShieldCheck className="h-3 w-3" />验证</Button>
                <Button size="sm" variant="ghost" disabled={!canWrite || deletion.disabled} onClick={() => onDelete(d)}><Trash2 className="h-3 w-3" />{deletion.label}</Button>
              </div>
            </article>
          );
        })}
...
function Routing({ policies, canWrite, onOpen }: { policies: DeliveryPolicyDraft[]; canWrite: boolean; onOpen: (id: string) => void }) {
  return (
    <div>
      <h2 className="text-sm font-semibold">投递路由</h2>
      <p className="mt-1 mb-3 text-xs text-[var(--text-muted)]">事件、目标组、主渠道与降级链通过版本化发布生效。</p>
      <div className="overflow-x-auto rounded-lg border border-[var(--border)]">
        <table className="min-w-[700px] w-full text-xs">
          <thead className="bg-[var(--bg-elevated)]">
            <tr>
              <th className="px-3 py-2.5 text-left">事件</th>
          ...
          <tbody>
            {policies.map((p) => (
              <tr key={p.id} className="border-t border-[var(--border)]">
                <td className="px-3 py-2.5 font-semibold">{p.eventType}</td>
                <td>{p.audience}</td>
                <td>{p.dataClassification}</td>
                <td><Badge tone={p.status === 'published' ? 'success' : p.status === 'ready' ? 'warn' : 'neutral'}>{deliveryPolicyStatusLabel(p.status)}</Badge></td>
                <td className="px-3 text-right"><Button size="sm" variant="ghost" disabled={!canWrite} onClick={() => onOpen(p.id)}>管理</Button></td>
              </tr>
            ))}
          </tbody>
        </table>
...
function Health({ overview }: { overview?: { capacityRisk: string } }) {
  return (
...
function Failures({ items }: { items: DeliveryAttempt[] }) {
  return (
    <div>
      <h2 className="text-sm font-semibold">失败处置</h2>
      <p className="mt-1 mb-3 text-xs text-[var(--text-muted)]">失败消息自动重试、降级后进入死信队列；载荷与目标均脱敏显示。</p>
      {items.length ? (
        <div className="space-y-2">
          {items.map((i) => (
            <div key={i.id} className="rounded-lg border border-[var(--danger)]/30 bg-[var(--danger-bg)] p-3 text-xs">
        ...
        <EmptyState icon={CheckCircle2} title="没有待处置死信" />
      )}
    </div>
  );
}
...
function Audit({ items }: { items: ChannelAuditEvent[] }) {
  return (
    <div>
      ...
      <p className="mt-1 mb-3 text-xs text-[var(--text-muted)]">接入、验证、发布、投递和失败处置均保留关联证据。</p>
      {items.length ? (
        <div className="space-y-2">
          {items.map((i) => (
            <article key={i.id} className="rounded-lg border border-[var(--border)] p-3 text-xs">
        ...
        <EmptyState icon={History} title="暂无渠道审计事件" />
      )}
    </div>
  );
}

function PolicyDrawer({ policy, versions, canWrite, onValidate, onPublish, onSimulate }: { policy: DeliveryPolicyDraft; versions: DeliveryPolicyVersion[]; canWrite: boolean; onValidate: () => void; onPublish: () => void; onSimulate: () => void }) {
  return (
    <div className="space-y-3 text-xs">
      <Panel title="策略范围" icon={Route} text={`${policy.eventType} → ${policy.audience} · ${policy.dataClassification}`} />
      {policy.validationIssues.length > 0 && <div className="rounded-lg border border-[var(--danger)]/30 bg-[var(--danger-bg)] p-3 text-[var(--danger)]">{policy.validationIssues.join('；')}</div>}
      ...
      <div className="border-t pt-3">
        <strong>版本历史</strong>
        {versions.length ? versions.map((v) => <p key={v.id} className="mt-2">v{v.version} · {new Date(v.publishedAt).toLocaleString('zh-CN')}</p>) : <p className="mt-2 text-[var(--text-muted)]">暂无已发布版本</p>}
      </div>
    </div>
  );
}
...
function Panel({ title, icon: Icon, text }: { title: string; icon: typeof Activity; text: string }) {
  return (
    <div className="rounded-lg border border-[var(--border)] p-3">
      <div className="flex items-center gap-2 font-semibold"><Icon className="h-4 w-4 text-[var(--brand)]" />{title}</div>
