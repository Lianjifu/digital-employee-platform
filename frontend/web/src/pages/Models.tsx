    ...
    <div className="models-page h-full overflow-y-auto bg-[var(--bg-elevated)] p-3 md:p-5">
      <main className="mx-auto max-w-[1440px] rounded-xl border border-[var(--border)] bg-[var(--bg)] shadow-[var(--shadow-xs)]" aria-label="模型中心">
        ...
        <section className="grid gap-3 border-b border-[var(--border)] bg-[var(--bg-elevated)]/40 p-4 sm:grid-cols-3 sm:px-6">
          <Metric label="可用供应商" value={String(governanceQuery.data?.activeProviders ?? 0)} tone="brand" />
      ...
      <Drawer open={providerDrawer === 'new'} onClose={() => setProviderDrawer(null)} title="接入供应商" description="凭据仅在提交时写入 Mock 凭据引用，成功后不会回显。" width={520}>
        <ProviderForm canWrite={canWrite} workspaceId={currentWorkspaceId} onSubmit={(payload) => createProvider.mutate(payload, { onSuccess: () => { toast.success('供应商已接入，等待连通性验证'); setProviderDrawer(null); }, onError: reportError })} />
      </Drawer>
      <Drawer open={Boolean(activeProvider)} onClose={() => setProviderDrawer(null)} title={activeProvider?.name} description="供应商详情、连通性和退役影响" width={520}>
        {activeProvider && <ProviderDetail provider={activeProvider} impact={impactQuery.data} canWrite={canWrite} onTest={() => testProvider.mutate({ id: activeProvider.id, reason: '人工连通性验证' }, { onSuccess: () => toast.success('供应商连通性验证通过'), onError: reportError })} onDisable={() => disableProvider.mutate({ id: activeProvider.id, reason: '停止新流量' }, { onSuccess: () => toast.success('供应商已停止新流量'), onError: reportError })} onDelete={() => setDeleteProvider(activeProvider)} />}
      </Drawer>
      ...
      <ConfirmDialog open={Boolean(deleteProvider)} onClose={() => setDeleteProvider(null)} onConfirm={() => { if (deleteProvider) removeProvider.mutate({ id: deleteProvider.id, reason: '人工确认删除' }, { onSuccess: () => toast.success('供应商已删除'), onError: reportError }); }} title="删除未被引用的供应商？" description="已被已发布路由引用的供应商将被 API 拒绝删除。" confirmText="删除" tone="danger" />
    </div>
  );
}
